...
export function runApp(app, rootElement) {
  return Rx.Observable.merge(
    app.html.debounce(1, Rx.Scheduler.requestAnimationFrame).scan(patch, rootElement),
    app.tasks
  ).subscribe();
}
...
export function App(config) {
  ...
  const stateWithEffects = Rx.Observable.just(modelWithEffect).concat(
    actions.scan(([model], action) => config.update(action, model), modelWithEffect)
  ).publish().refCount();

  const effectToTask = pipe(nth(1), effect => effect.map(action => send(address, action)));

  const tasks = stateWithEffects.flatMap(effectToTask).publish().refCount();
